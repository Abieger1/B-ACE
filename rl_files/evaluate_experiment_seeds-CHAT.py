...
def load_model_with_pt_fallback(
    ...
    """
    Load a PPO model, falling back to .pt files if .zip is corrupt/empty.
    
    Args:
        model_path: Path to the model (typically best_model.zip)
    ...
    Returns:
        Loaded PPO model
    """
    # First, try loading from .zip file
    try:
        ...
        if model_path.exists() and model_path.stat().st_size > 0:
            ...
            raise ValueError(f"Model file is empty or missing: {model_path}")
    except Exception as zip_error:
        if verbose:
            print(f"   ⚠ Failed to load {model_path.name}: {zip_error}")
        
        # Look for .pt file fallback
        model_dir = model_path.parent
        
        # Try different .pt file naming conventions
        pt_candidates = [
            model_dir / "best_policy.pt",
            model_dir / "policy.pt",
            model_dir / (model_path.stem.replace("model", "policy") + ".pt"),  # best_model -> best_policy
        ]
        
        policy_pt_path = None
        for pt_path in pt_candidates:
            if pt_path.exists() and pt_path.stat().st_size > 0:
                policy_pt_path = pt_path
                break
        
        if policy_pt_path is None:
            raise FileNotFoundError(
                f"Could not load model from {model_path} and no valid .pt fallback found. "
                f"Checked: {[p.name for p in pt_candidates]}"
            )
        ...
        if isinstance(checkpoint, dict):
            # Check for nested state dict (common format from custom callbacks)
            if "policy_state_dict" in checkpoint:
                state_dict = checkpoint["policy_state_dict"]
                if verbose:
                    print(f"   → Found nested policy_state_dict (timestep: {checkpoint.get('timestep', 'N/A')}, "
                          f"mean_reward: {checkpoint.get('mean_reward', 'N/A')})")
        ...
        if verbose:
            print(f"   ✓ Loaded policy weights from {policy_pt_path.name}")
...
def find_seed_models(experiment_dir: Path) -> Dict[str, Tuple[Path, Path]]:
    ...
    Note: model_path may point to a .zip file (even if empty/corrupt) or a .pt file.
    The load_model_with_pt_fallback function handles the actual loading.
    """
    ...
    for seed_dir in sorted(experiment_dir.glob("*seed*")):
        ...
        # Find best model - look for .zip first, then .pt as fallback
        model_candidates = [
            seed_dir / "best_model.zip",
            seed_dir / "best_checkpoint" / "best_model.zip",
            seed_dir / "eval_checkpoints" / "best_model.zip",
        ]
        
        # Also check for .pt files as valid model sources
        pt_fallback_candidates = [
            seed_dir / "best_policy.pt",
            seed_dir / "best_checkpoint" / "best_policy.pt",
            seed_dir / "eval_checkpoints" / "best_policy.pt",
        ]
        
        model_path = None
        using_pt_fallback = False
        
        # First, look for .zip files (even if empty - the loader will handle fallback)
        for c in model_candidates:
            if c.exists():
                model_path = c
                break
        
        # If no .zip found, check if we have .pt files
        if model_path is None:
            for c in pt_fallback_candidates:
                if c.exists() and c.stat().st_size > 0:
                    # Use the directory's expected .zip path, loader will find the .pt
                    model_path = c.parent / "best_model.zip"
                    using_pt_fallback = True
                    break
        
        if model_path is None:
            print(f"Warning: No best_model.zip or best_policy.pt found for {seed_dir.name}")
            print(f"   Checked .zip: {[str(c.relative_to(seed_dir)) for c in model_candidates]}")
            print(f"   Checked .pt:  {[str(c.relative_to(seed_dir)) for c in pt_fallback_candidates]}")
            continue
        ...
        vecnorm_candidates = [
            ...
            seed_dir / "best_checkpoint" / "best_vecnormalize.pkl",
            seed_dir / "best_checkpoint" / "vecnormalize.pkl",
        ...
        if vecnorm_path is None:
            ...
            print(f"   Checked: {[str(c.relative_to(seed_dir)) for c in vecnorm_candidates]}")
            continue
        
        results[seed_name] = (model_path, vecnorm_path)
        # Show relative paths for clarity
        model_rel = model_path.relative_to(seed_dir)
        vecnorm_rel = vecnorm_path.relative_to(seed_dir)
        
        # Note if we're using .pt fallback
        note = " (will use .pt fallback)" if using_pt_fallback else ""
        if model_path.exists() and model_path.stat().st_size == 0:
            note = " (empty .zip, will use .pt fallback)"
        
        print(f"Found {seed_name}: {model_rel}, {vecnorm_rel}{note}")
...
def evaluate_model(
    ...
    # Load model (with fallback to .pt files if .zip is corrupt)
    model = load_model_with_pt_fallback(model_path, eval_env, verbose=verbose)
