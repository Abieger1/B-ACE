...
def plot_eval_learning_curve(
    ...
    if n_seeds > 1:
        ...
        for seed_name, (ts, means, stds, n_eval_eps) in sorted(seed_data.items()):
            ...
            seed_peak = float(np.max(means))
            seed_auc = float(np.mean(means))  # For evenly spaced evals, AUC = mean
        ...
        # Compute metrics for single seed
        mean_peak = float(np.max(avgEval))
        mean_auc = float(np.mean(avgEval))
        peak_idx = int(np.argmax(avgEval))
        ...
        if stds is not None and n_eval_eps and n_eval_eps > 1:
            ...
            # CI for shaded region (per-timestep)
            ...
            peak_se = stds_arr[peak_idx] / np.sqrt(n_eval_eps)
            peak_hw = float(t.ppf(0.975, n_eval_eps - 1) * peak_se)
            
            # CI for AUC: use pooled variance across all checkpoints
            # Mean of the SEs gives us an estimate of uncertainty in the average
            mean_se = float(np.mean(stds_arr)) / np.sqrt(n_eval_eps)
