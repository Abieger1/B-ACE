...
from hierarchical_action_wrapper_fsm_FIXED import HierarchicalFSMActionWrapper, CurriculumConfig
...
class FireCooldownWrapper(gym.Wrapper):
    """
    Action-side safety gate:
      - optional rising-edge fire (one shot per press)
      - hard cooldown between shots (prevents 4 missiles in 4 timesteps)
    """
    FIRE_IDX = 3

    def __init__(self, env, cooldown_steps: int = 80, require_rising_edge: bool = True):
        super().__init__(env)
        self.cooldown_steps = int(cooldown_steps)
        self.require_rising_edge = bool(require_rising_edge)

        self._t = 0
        self._last_fire_t = -10**9
        self._prev_fire_req = False
    ...
    def reset(self, **kwargs):
        ...
        if isinstance(out, tuple) and len(out) == 2:
            return out
        # Old gym reset returns obs only
        return out, {}

    def step(self, action):
        a = np.asarray(action, dtype=np.float32).copy()

        want_fire = float(a[self.FIRE_IDX]) > 0.5

        # Rising-edge: only allow fire on 0->1 transition
        if self.require_rising_edge:
            rising = want_fire and (not self._prev_fire_req)
            self._prev_fire_req = want_fire
            if not rising:
                a[self.FIRE_IDX] = 0.0
                want_fire = False
        else:
        ...
        # Cooldown: block if last shot too recent
        if want_fire and (self._t - self._last_fire_t) < self.cooldown_steps:
            a[self.FIRE_IDX] = 0.0
            want_fire = False

        # If fire is accepted, make it a one-step pulse
        if want_fire:
            a[self.FIRE_IDX] = 1.0
...
def main():
    ...
    def make_env(log_dir_str, seed, is_eval: bool = False):
        def _thunk():
            ...
            if args.use_enriched_obs:
                ...
                e = FireCooldownWrapper(e, cooldown_steps=80, require_rising_edge=True)
                print("✓ FireCooldownWrapper added")
                ...
                e = HierarchicalFSMActionWrapper(
                    e,
                    curriculum_config=CurriculumConfig(
                        # These curriculum values matter only for the wrapper's internal config.
                        # Expert mixing is controlled by ExpertMixPPO.greedy_prob_fn in training.
                        initial_greedy_prob=1.0,
                        final_greedy_prob=1.0,
                        curriculum_steps=1,
                    ),
                    debug=False,
                )
                print("✓ HierarchicalFSMActionWrapper added (train/eval)")
    ...
    def greedy_prob_fn(t: int) -> float:
        p_start = 0.01
        p_end   = 0.95       # Cap at 70% - always keep 30% expert
    ...
    model = ExpertMixPPO(
        policy="MlpPolicy",
        env=vec_env,
        greedy_prob_fn=greedy_prob_fn,
        use_fire_gate=False,
        bc_coef_fn=bc_coef_fn,                  # NEW
        bc_use_gated_expert=True,               # NEW (recommended)
        rollout_buffer_class=ExpertRolloutBuffer, # NEW
