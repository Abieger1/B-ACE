#!/usr/bin/env python3
"""
evaluate_single_model_multi_seed.py (FIXED)

Evaluate a SINGLE trained model across multiple evaluation seeds.
AUTO-DETECTS observation dimension from saved VecNormalize stats.

FIXED: Restored correct info key extraction from the working old version.
...
def evaluate_with_seed(
    ...
    eval_seed: int,
    n_episodes: int,
    obs_mode: str,  # "baseline", "enriched", or "enriched_extended"
    ...
    def make_env():
        def _thunk():
            ...
            # Get baseline obs dim
            baseline_obs_dim = e.observation_space.shape[0]
            ...
            actual_dim = e.observation_space.shape[0]
            if actual_dim != expected_obs_dim:
                print(f"  WARNING: Obs dim mismatch! Expected {expected_obs_dim}, got {actual_dim}")
                print(f"           obs_mode={obs_mode}, baseline_dim={baseline_obs_dim}")
    ...
    while finished < n_episodes:
        ...
        # FIXED: Safely extract info dict
        info = infos[0] if isinstance(infos, (list, tuple)) else (infos if isinstance(infos, dict) else {})
        
        # FIXED: Track events using CORRECT key names from old working version
        # Check multiple possible keys for HVAA status
        if _as_bool(info.get("hvaa_destroyed", False)) or not _as_bool(info.get("hvaa_alive", True)):
            ep_hvaa_destroyed = True
        
        # Check multiple possible keys for red kill status
        ...
        # Check multiple possible keys for blue kill status
        ...
        if bool(done[0]):
            # FIXED: Extract episode data using OLD working approach
            # Get values directly from final info dict, not from step-by-step tracking
            episode = EpisodeData(
                reward=cur_reward,
                length=cur_length,
                ...
                # FIXED: Get PK directly from info dict (env computes it)
                ...
                # FIXED: Check multiple possible keys for termination reason
...
def compute_results(all_episodes: List[EpisodeData], eval_seeds: List[int], episodes_per_seed: int) -> Dict:
    ...
    hvaa_survived = [ep.hvaa_survived for ep in all_episodes]
    ...
    for seed in eval_seeds:
        ...
        per_seed[str(seed)] = {
            ...
            "std_reward": float(np.std([ep.reward for ep in seed_eps])),
    ...
    return {
        ...
        "confidence_intervals": {
            ...
            "hvaa_survival_rate": asdict(wilson_score_interval(sum(hvaa_survived), n)),
...
def parse_args():
    ...
    # Observation mode - can override auto-detection
    p.add_argument("--obs-mode", type=str, choices=["auto", "baseline", "enriched", "enriched_extended"],
                   default="auto",
                   help="Observation mode: 'auto' detects from vecnormalize, or force 'baseline'/'enriched'")
...
def main():
    ...
    if args.obs_mode == "auto":
        ...
        print(f"\n🔍 Auto-detected observation mode:")
        print(f"   VecNormalize obs_dim: {expected_obs_dim}")
        print(f"   Determined mode: {obs_mode}")
    else:
        obs_mode = args.obs_mode
        print(f"\n⚙️  Using forced observation mode: {obs_mode}")
        print(f"   VecNormalize obs_dim: {expected_obs_dim}")
    ...
    print("\n" + "="*80)
    print("SINGLE MODEL MULTI-SEED EVALUATION (FIXED)")
    print("="*80)
    print(f"Model: {model_path}")
    print(f"VecNormalize: {vecnorm_path}")
    print(f"Obs mode: {obs_mode} (dim={expected_obs_dim})")
    ...
    for i, eval_seed in enumerate(eval_seeds):
        ...
        red_kill_rate = np.mean([ep.red_killed for ep in episodes])
        print(f"      Seed {eval_seed} summary: reward={mean_r:.2f}, HVAA={hvaa_rate*100:.1f}%, Red Kill={red_kill_rate*100:.1f}%")
