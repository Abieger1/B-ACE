...
- Number of epochs
- Mini-batch size
- Neural network architecture (layers and size)
"""
...
def objective(trial: optuna.Trial, config_path: Path, total_timesteps: int, n_eval_episodes: int = 10) -> float:
    ...
    initial_entropy = trial.suggest_float("initial_entropy", 1e-3, 8e-2)
    final_entropy = trial.suggest_float("final_entropy", 0.0, initial_entropy)
    
    # Constant clip range (not decaying - PPO best practice)
    clip_range = trial.suggest_categorical("clip_range", [0.1, 0.15, 0.2, 0.25, 0.3])
    
    vf_coef = trial.suggest_float("vf_coef", 0.3, 0.9)
    gae_lambda = trial.suggest_float("gae_lambda", 0.90, 0.99)  # NEW: GAE lambda
    ...
    print(f"GAE Lambda: {gae_lambda:.3f}")
    print(f"Epochs: {n_epochs}")
    print(f"Batch Size: {batch_size}")
    print(f"Architecture: {n_layers} layers x {layer_size} neurons")
    ...
    # Build neural network architecture based on n_layers
    if n_layers == 2:
        net_arch = dict(pi=[layer_size, layer_size], vf=[layer_size, layer_size])
    else:  # n_layers == 3
        net_arch = dict(pi=[layer_size, layer_size, layer_size], vf=[layer_size, layer_size, layer_size])
    ...
    model = PPO(
        ...
        n_epochs=n_epochs,
        gamma=0.99,
        gae_lambda=gae_lambda,  # Now optimized
