...
def analyze_study(study_name: str, storage: str):
    ...
    print(f"\n{'Trial':<8} {'Value':<12} {'Init LR':<12} {'Final LR':<12} {'Batch':<10} {'Epochs':<8} {'Layers':<8} {'Size':<8}")
    print(f"{'-'*80}")
    for idx, row in df_complete.head(10).iterrows():
        trial_num = int(row['number'])
        value = row['value']
        init_lr = row.get('params_initial_lr', row.get('params_learning_rate', 'N/A'))
        final_lr = row.get('params_final_lr', 'N/A')
        batch_size = row.get('params_batch_size', 'N/A')
        epochs = row.get('params_n_epochs', 'N/A')
        n_layers = row.get('params_n_layers', 'N/A')
        layer_size = row.get('params_layer_size', 'N/A')
        ...
        if isinstance(final_lr, float):
            final_lr_str = f"{final_lr:.2e}"
        else:
            final_lr_str = str(final_lr)
        
        print(f"{trial_num:<8} {value:<12.4f} {init_lr_str:<12} {final_lr_str:<12} {batch_size!s:<10} {epochs!s:<8} {n_layers!s:<8} {layer_size!s:<8}")
...
def visualize_study(study_name: str, storage: str, output_dir: Path = None):
    ...
    # Contour plot for architecture
    try:
        fig = optuna.visualization.plot_contour(study, params=['n_layers', 'layer_size'])
        fig.write_html(str(output_dir / "contour_architecture.html"))
        print("  ✓ contour_architecture.html")
    except Exception as e:
        print(f"  ✗ Could not create architecture contour plot: {e}")
...
def generate_command(study_name: str, storage: str, output_file: Path = None):
    ...
    # Handle learning rate (with or without decay)
    ...
    # Handle entropy coefficient (with or without decay)
    if 'initial_entropy' in params:
        ...
        cmd += f" --entropy-coef {params['entropy_coef']:.6f}"
    
    # Clip range (always constant)
    ...
    if 'gae_lambda' in params:
        cmd += f" --gae-lambda {params['gae_lambda']:.6f}"
    ...
    if 'n_layers' in params and 'layer_size' in params:
        n_layers = params['n_layers']
        layer_size = params['layer_size']
        cmd += f" --n-layers {n_layers}"
        cmd += f" --layer-size {layer_size}"
