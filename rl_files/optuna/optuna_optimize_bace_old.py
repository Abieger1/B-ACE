...
def objective(trial: optuna.Trial, config_path: Path, total_timesteps: int, n_eval_episodes: int = 10) -> float:
    ...
    try:
        # Train model (with normalized rewards for stable learning)
        model.learn(
        ...
        # ============================================================
        # Evaluate model with RAW (unnormalized) rewards
        # This ensures fair comparison across Optuna trials
        # ============================================================
        
        # Create a fresh evaluation env WITHOUT reward normalization
        eval_env = DummyVecEnv([make_env])
        eval_env = VecNormalize(
            eval_env,
            norm_obs=True,       # Keep obs normalization (model expects normalized obs)
            norm_reward=False,   # DON'T normalize rewards - we want true values
            clip_obs=10.0,
        )
        
        # Copy observation normalization statistics from training env
        # This ensures the model sees observations in the same scale it was trained on
        ...
        for _ in range(n_eval_episodes):
            ...
            while not done:
                ...
                episode_reward += reward[0]  # This is now the TRUE unnormalized reward
                if done:
        ...
        print(f"  Raw Mean Reward: {mean_reward:.2f} ± {std_reward:.2f}")
...
def main():
    ...
    parser.add_argument("--timesteps-per-trial", type=int, default=1_000_000,
