#!/usr/bin/env python3
"""
train_bace_config2.py - Scenario 1, Configuration 2: Expert Alignment Reward

Training script with expert alignment bonus that rewards PPO agent for 
choosing actions similar to the SimpleAggressiveExpert's recommendations.

Key features:
- Expert alignment reward: Agent receives bonus for matching expert actions
- Alignment coefficient with decay: Starts strong, decays over training
- No action injection: Agent always executes its own actions
- Enriched observations (47 dims) with pursuit-evasion domain knowledge

Usage:
    # Run with expert alignment (Configuration 2)
    python train_bace_config2.py --expert-alignment --seed 42
    
    # With custom alignment coefficient
    python train_bace_config2.py --expert-alignment --alignment-coef 0.15 --seed 42
    
    # Without expert alignment (baseline)
    python train_bace_config2.py --seed 42
"""
            ...
            if args.use_enriched_obs:
                ...
                if args.expert_alignment:
                    ...
                    e = ExpertAlignmentWrapper(
                        ...
                        action_weights=np.array([1.0, 0.3, 0.5, 0.6]),  # turn, alt, g, fire
                        decay_fn=alignment_decay,
                        debug=False,
                    )
                    print(f"✓ Expert alignment wrapper added (coef={args.alignment_coef})")
