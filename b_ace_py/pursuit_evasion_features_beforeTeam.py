...
class PursuitEvasionFeatures:
    ...
    def compute_safe_heading_cone(self,
        ...
        Returns:
            Dict with safe heading cone information
        """
        if not threats:
            return {
                'safe_cone_width': 2 * np.pi,
                'safe_cone_center': agent_heading,
                'num_threats_active': 0,
                'fully_surrounded': False
            }
        
        # Collect unsafe angular intervals from each threat
        unsafe_intervals = []
        
        for threat in threats:
            threat_pos = np.array(threat['position'][:2])
            
            dmc_result = self.compute_dmc(agent_pos, agent_heading, 
                                          threat_pos, mu, R, r)
            ...
            # Unsafe headings are those with |aspect_angle| < ξ*
            unsafe_min = los_angle - xi_star
            unsafe_max = los_angle + xi_star
            
            unsafe_intervals.append((unsafe_min, unsafe_max))
        ...
        # Merge overlapping intervals and find safe cone
        # (Simplified: just report statistics)
        total_unsafe = sum(max - min for min, max in unsafe_intervals)
        safe_width = max(0, 2 * np.pi - total_unsafe)
        
        return {
            'safe_cone_width': safe_width,
            'safe_cone_width_normalized': safe_width / (2 * np.pi),
            'num_threats_active': len(unsafe_intervals),
            'fully_surrounded': safe_width < 0.1,  # Nearly surrounded
            'total_unsafe_arc': total_unsafe
        }
